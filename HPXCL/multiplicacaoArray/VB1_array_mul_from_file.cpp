...
int main(int argc, char* argv[]) {
	...
	buffer inbuffer_2 = cudaDevice_1.create_buffer(sizeof(int) * SIZE).get();

	data_futures_2.push_back(inbuffer_2.enqueue_write(0, sizeof(int) * SIZE, input_2));

	program prog_2 = cudaDevice_1.create_program_with_file("array_mul_kernel.cu").get();


	std::vector<std::string> flags_2;
	std::string mode_2 = "--gpu-architecture=compute_";
	mode_2.append(std::to_string(cudaDevice_1.get_device_architecture_major().get()));
	mode_2.append(std::to_string(cudaDevice_1.get_device_architecture_minor().get()));
	...
	buffer outbuffer_2 = cudaDevice_1.create_buffer(sizeof(int) * SIZE).get();
	...
	buffer sizebuffer_2 = cudaDevice_1.create_buffer(sizeof(int)).get();
